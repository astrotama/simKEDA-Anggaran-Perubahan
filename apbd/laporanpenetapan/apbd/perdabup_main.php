							...
							if ($perdabup == 'perbup') {
								...
								if ($resulto) {
									while ($datao = db_fetch_object($resulto)) {
										...
										//RINCIAN OBYEK
										$sql = 'select ro.kodero,ro.uraian,jumlah jumlahx, k.ketrekening from {anggperuk} k inner join {rincianobyek} ro on k.kodero=ro.kodero where k.kodeuk=\'%s\' and ro.kodeo=\'%s\' and jumlah>0';
										$fsql = sprintf($sql, db_escape_string($kodeuk), db_escape_string($datao->kodeo));
										$fsql .= ' order by ro.kodero';
										//drupal_set_message( $fsql);
										$result = db_query($fsql);
		...
		while ($dataakun = db_fetch_object($resultakun)) {
			...
			if ($resultkel) {
				while ($datakel = db_fetch_object($resultkel)) {
					...
					//JENIS
					$sql = 'select j.kodej,j.uraian,sum(jumlah) jumlahx from {anggperkeg} k inner join {jenis} j on mid(k.kodero,1,3)=j.kodej  inner join {kegiatanskpd} g on k.kodekeg=g.kodekeg where g.inaktif=0 and g.kodeuk=\'%s\' and j.kodek=\'%s\' and jumlah>0';
					$fsql = sprintf($sql, db_escape_string($kodeuk), db_escape_string($datakel->kodek));
					$fsql .= ' group by j.kodej,j.uraian order by j.kodej';
					...
					if ($resultj) {
						while ($dataj = db_fetch_object($resultj)) {
							...
							if ($perdabup == 'perbup') {
								//OBYEK
								$sql = 'select o.kodeo,o.uraian,sum(jumlah) jumlahx from {anggperkeg} k inner join {obyek} o on mid(k.kodero,1,5)=o.kodeo inner join {kegiatanskpd} g on k.kodekeg=g.kodekeg where g.inaktif=0 and g.kodeuk=\'%s\' and o.kodej=\'%s\' and jumlah>0';
								$fsql = sprintf($sql, db_escape_string($kodeuk), db_escape_string($dataj->kodej));
								$fsql .= ' group by o.kodeo,o.uraian order by o.kodeo';
								...
								if ($resulto) {
									while ($datao = db_fetch_object($resulto)) {
										...
										//RINCIAN OBYEK
										$sql = 'select ro.kodero,ro.uraian,sum(jumlah) jumlahx from {anggperkeg} k inner join {rincianobyek} ro on k.kodero=ro.kodero inner join {kegiatanskpd} g on k.kodekeg=g.kodekeg where g.inaktif=0 and g.kodeuk=\'%s\' and ro.kodeo=\'%s\' and jumlah>0';
										$fsql = sprintf($sql, db_escape_string($kodeuk), db_escape_string($datao->kodeo));
										$fsql .= ' group by ro.kodero,ro.uraian order by ro.kodero';
										...
										if ($result) {
											while ($dataro = db_fetch_object($result)) {
												...
												if (($dataj->kodej == 516) or ($dataj->kodej == 517) ) {
													//KEGIATAN
													$sql = 'select k.kodekeg,k.kegiatan,a.jumlah from {kegiatanskpd} k inner join {anggperkeg} a on k.kodekeg=a.kodekeg where a.kodero=\'%s\' and a.jumlah>0 order by kegiatan';
													$fsql = sprintf($sql, db_escape_string($dataro->kodero));
			...
			if ($resultkel) {
				while ($datakel = db_fetch_object($resultkel)) {
					...
					inner join {program} p on k.kodepro=p.kodepro where k.inaktif=0 and k.kodeuk=\'%s\' and k.jenis=2 and k.total>0';
					$fsql = sprintf($sql, db_escape_string($kodeuk));
					$fsql .= ' group by k.kodepro,p.program order by k.kodepro';			
					$resultpro = db_query($fsql);
					if ($resultpro) {
						while ($datapro = db_fetch_object($resultpro)) {
							...
							//KEGIATAN
							$sql = 'select kodekeg, nomorkeg, kegiatan uraian, total jumlahx, lokasi, sumberdana1 from {kegiatanskpd} k where k.inaktif=0 and k.kodeuk=\'%s\' and k.kodepro=\'%s\' and k.jenis=2  and total>0';
							$fsql = sprintf($sql, db_escape_string($kodeuk), db_escape_string($datapro->kodepro));
							$fsql .= ' order by k.nomorkeg';			
							$resultkeg = db_query($fsql);
							if ($resultkeg) {
	...
	if ($kodeuk=='81') {
		...
		if ($resultkel) {
			while ($datakel = db_fetch_object($resultkel)) {
				...
				//JENIS
				$sql = 'select j.kodej,j.uraian,sum(k.jumlah) jumlahx from {anggperda} k inner join {jenis} j on mid(k.kodero,1,3)=j.kodej where mid(k.kodero,1,2)=\'%s\' and k.jumlah>0';
				$fsql = sprintf($sql, db_escape_string($datakel->kodek));
				$fsql .= ' group by j.kodej,j.uraian order by j.kodej';
				...
				if ($resultj) {
					while ($dataj = db_fetch_object($resultj)) {
						...
						if ($perdabup == 'perbup') {
							//OBYEK
							$sql = 'select o.kodeo,o.uraian,sum(k.jumlah) jumlahx from {anggperda} k inner join {obyek} o on mid(k.kodero,1,5)=o.kodeo where o.kodej=\'%s\' and k.jumlah>0';
							$fsql = sprintf($sql, db_escape_string($dataj->kodej));
							$fsql .= ' group by o.kodeo,o.uraian order by o.kodeo';
							//drupal_set_message( $fsql);
							$resulto = db_query($fsql);
							if ($resulto) {
								while ($datao = db_fetch_object($resulto)) {
									...
									//RINCIAN OBYEK
									$sql = 'select ro.kodero,ro.uraian,k.jumlah jumlahx from {anggperda} k inner join {rincianobyek} ro on k.kodero=ro.kodero where ro.kodeo=\'%s\' and k.jumlah>0';
									$fsql = sprintf($sql, db_escape_string($datao->kodeo));
									$fsql .= ' order by ro.kodero';
									//drupal_set_message( $fsql);
									$result = db_query($fsql);
