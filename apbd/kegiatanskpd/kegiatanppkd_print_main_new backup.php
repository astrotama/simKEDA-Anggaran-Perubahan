...
drupal_add_css('files/css/kegiatancam.css');
	...
	//drupal_set_message(variable_get('apbdrevisi', 0));
	...
	if (isset($exportpdf) && ($exportpdf=='pdf'))  {
		...
		$pdfFile = 'rka-ppkd-kegiatan_1.pdf';
		...
		//	$htmlContent = GenReportFormContentRKA();
		//else
		$htmlContent = GenReportFormContent1($revisi);
		$htmlFooter = '';
		...
		$pdfFile = 'rka-ppkd-kegiatan_2.pdf';


			
		$htmlHeader = '';
		$htmlContent = GenReportFormContent2($revisi);
		$htmlFooter = '';		//GenReportFormFooter($revisi);
		...
		$pdfFile = 'rka-ppkd-kegiatan_3.pdf';


			
		$htmlHeader = '';
		$htmlContent = GenReportFormContent3($revisi);
		$htmlFooter = GenReportFormFooter($revisi);
...
function GenReportFormContent1($revisi) {
	...
	//KELOMPOK
	$where = sprintf(' where left(k.kodero,2)=\'%s\' and left(k.kodero,3)>\'%s\' and left(k.kodero,3)<=\'%s\' ', db_escape_string('51'), db_escape_string('511'), db_escape_string('516'));
	$sql = 'select l.kodek,l.uraian,sum(k.jumlah) jumlahx, sum(k.jumlahp) jumlahxp from {anggperkegperubahan' . $str_table . '} k  inner join {kelompok} l on mid(k.kodero,1,2)=l.kodek ' . $where;
	$sql .= ' group by l.kodek,l.uraian order by l.kodek';
	...
	if ($resultkel) {
		while ($datakel = db_fetch_object($resultkel)) {
			...
			//JENIS	
			$where = sprintf(' where left(k.kodero,2)=\'%s\' and left(k.kodero,2)=\'%s\' and left(k.kodero,3)>\'%s\'  and left(k.kodero,3)<=\'%s\'',  
			db_escape_string($datakel->kodek), db_escape_string('51'), db_escape_string('511'), db_escape_string('516'));
			$sql = 'select mid(k.kodero,1,3) kodej,j.uraian,sum(jumlah) jumlahx,sum(jumlahp) jumlahxp from {anggperkegperubahan' . $str_table . '} k  left join {jenis} j on mid(k.kodero,1,3)=j.kodej ' . $where;
			$sql .= ' group by mid(k.kodero,1,3),j.uraian order by mid(k.kodero,1,3)';
	...
	$rowsrek[] = array (
						 array('data' => '',  'width'=> '280px',  'colspan'=>'2',  'style' => 'border-top: 1px solid black; text-align:right; font-weight:bold;'),
						 array('data' => '', 'width' => '60px', 'style' => ' border-top: 1px solid black;text-align:center;font-size:small;'),
						 array('data' => '', 'width' => '60px', 'style' => ' border-top: 1px solid black; text-align:center;font-size:small;'),
						 array('data' => '',  'width' => '60px', 'style' => ' border-top: 1px solid black; text-align:center;font-size:small;'),
						 array('data' => '',  'width' => '65px', 'style' => ' border-top: 1px solid black; text-align:right;font-size:small;'),
						 ...
						 array('data' => '',  'width' => '70px', 'style' => ' border-top: 1px solid black;  text-align:right;font-size:small;'),
						 array('data' => '',  'width' => '35px', 'style' => ' border-top: 1px solid black;  text-align:right;font-size:small;'),
						 );
...
function GenReportFormContent2($revisi) {
	...
	//JENIS	
	//$where = sprintf(' where (left(k.kodero,3)=\'%s\' or left(k.kodero,3)=\'%s\')',  
	//db_escape_string('517'), db_escape_string('518'));
	$where = sprintf(' where left(k.kodero,5)=\'%s\'',  
	db_escape_string('51703'));
	$sql = 'select mid(k.kodero,1,3) kodej,j.uraian,sum(jumlah) jumlahx,sum(jumlahp) jumlahxp from {anggperkegperubahan' . $str_table . '} k  left join {jenis} j on mid(k.kodero,1,3)=j.kodej ' . $where;
	$sql .= ' group by mid(k.kodero,1,3),j.uraian order by mid(k.kodero,1,3)';
	...
	if ($resultjenis) {
		while ($datajenis = db_fetch_object($resultjenis)) {
			...
			$sql = 'select mid(k.kodero,1,5) kodeo,o.uraian,sum(jumlah) jumlahx,sum(jumlahp) jumlahxp from {anggperkegperubahan' . $str_table . '} k  left join {obyek} o on mid(k.kodero,1,5)=o.kodeo where mid(k.kodero,1,5)=\'%s\'';
			$fsql = sprintf($sql, db_escape_string('51703'));
			$fsql .= ' group by mid(k.kodero,1,5),o.uraian order by mid(k.kodero,1,5)';
			...
			if ($resultobyek) {
				while ($dataobyek = db_fetch_object($resultobyek)) {
					...
					//REKENING
					$sql = 'select kodero,uraian,sum(jumlah) jumlahx ,sum(jumlahp) jumlahxp from {anggperkegperubahan' . $str_table . '} k where mid(k.kodero,1,5)=\'%s\' and k.kodero<=\'%s\'';
					$fsql = sprintf($sql, db_escape_string($dataobyek->kodeo), db_escape_string('51703099'));
					$fsql .= ' group by kodero,uraian order by kodero';
...
function GenReportFormContent3($revisi) {
	...
	//REKENING
	$sql = 'select kodero,uraian,sum(jumlah) jumlahx ,sum(jumlahp) jumlahxp from {anggperkegperubahan' . $str_table . '} k where mid(k.kodero,1,5)=\'%s\' and k.kodero>\'%s\'';
	$fsql = sprintf($sql, db_escape_string('51703'), db_escape_string('51703099'));
	$fsql .= ' group by kodero,uraian order by kodero';
	...
	//$where = sprintf(' where (left(k.kodero,3)=\'%s\' or left(k.kodero,3)=\'%s\')',  
	//db_escape_string('517'), db_escape_string('518'));
	$where = sprintf(' where left(k.kodero,3)=\'%s\'',  
	db_escape_string('518'));
	$sql = 'select mid(k.kodero,1,3) kodej,j.uraian,sum(jumlah) jumlahx,sum(jumlahp) jumlahxp from {anggperkegperubahan' . $str_table . '} k  left join {jenis} j on mid(k.kodero,1,3)=j.kodej ' . $where;
	$sql .= ' group by mid(k.kodero,1,3),j.uraian order by mid(k.kodero,1,3)';
	...
	//TOTAL
	$total = 0; $totalp = 0;
	$where = sprintf(' where left(kodero,2)=\'%s\' and left(kodero,3)>\'%s\' ', db_escape_string('51'), db_escape_string('511'));
	$sql = 'select sum(jumlah) jumlahx, sum(jumlahp) jumlahxp from {anggperkegperubahan' . $str_table . '} ' . $where;
	$resulttot = db_query($sql);
	if ($resulttot) {
		while ($datatot = db_fetch_object($resulttot)) {
			$total = $datatot->jumlahx;
			$totalp = $datatot->jumlahxp;
		}
	}	
	$rowsrek[] = array (
						 array('data' => 'JUMLAH BELANJA',  'width'=> '280px',  'colspan'=>'2',  'style' => 'border-left: 1px solid black; border-bottom: 1px solid black; border-right: 1px solid black; border-top: 1px solid black; text-align:right; font-weight:bold;'),
