...
function GenReportFormHeader($kodekeg, $tipedok) {
	...
	$pquery = sprintf('select k.kodekeg, k.lokasi, k.programsasaran, k.programtarget, k.masukansasaran, k.masukantarget, k.keluaransasaran, k.keluarantarget, 
				...
				k.sumberdana1, k.sumberdana2, k.sumberdana1rp, k.sumberdana2rp, k.latarbelakang, k.kelompoksasaran, k.anggaran 
...
//
								...
								if ($bintangkegiatan) {
									...
									if (($datapen->anggaran==0) and ($datapen->jumlah>0)) {
		...
		while ($datajenis = db_fetch_object($resultjenis)) {
			...
			if ($resultobyek) {
				while ($dataobyek = db_fetch_object($resultobyek)) {
					...
					//REKENING
					$sql = 'select kodero,uraian,jumlah,anggaran from {anggperkegrevisi} k where kodekeg=\'%s\' and mid(k.kodero,1,5)=\'%s\'';
					$fsql = sprintf($sql, db_escape_string($kodekeg), db_escape_string($dataobyek->kodeo));
					...
					if ($result) {
						...
						if ($bintangkegiatan) {
							...
							} elseif ($data->anggaran < $data->jumlah) {
						...
						$rowsrek[] = array (
							...
							//DETIL
							$sql = 'select distinct iddetil,uraian,unitjumlah,unitsatuan,volumjumlah,volumsatuan,harga,total, pengelompokan, anggaran from {anggperkegdetilrevisi} where kodekeg=\'%s\' and kodero=\'%s\' order by nourut asc,iddetil';
							$fsql = sprintf($sql, db_escape_string($kodekeg), db_escape_string($data->kodero));
							////////drupal_set_message($fsql);
													...
													if ($datasuburaian_pen == $uraian_rev) {
														...
														if ($datasuburaian_pen == $datasub->uraian) {
															...
															if (trim($datasuburaian_pen) == trim($datasub->uraian)) {
																...
																if (($datasub->total>0) and  ($datasub->anggaran==0)) $statusdetilsub = '<font color="Red">*</font>';
