...
function kegiatanskpdover_main($arg=NULL, $nama=NULL) {
	...
	if (isset($exportpdf) && ($exportpdf=='pdf'))  {
		...
		if ($jenis=='0') {
			drupal_set_title('Daftar Usulan Perubahan yang Melebihi Plafon');
			$output = GenDataView();
		} else {
			drupal_set_title('Daftar Kegiatan yang Melebihi Plafon dan Belum Diusulkan');
			$output = GenDataViewBelumUsul();
		}
		return $output;
	}
}
...
function GenDataView() {
    ...
    $where = ' where k.total>k.plafon ' . $customwhere;

	$sql = "select k.kodekeg,k.nomorkeg,k.tahun,k.kodeuk,k.kegiatan,k.lokasi,
			k.programtarget,k.total, k.plafon,u.namasingkat, k.adminok, k.sumberdana1 sumberdana, k.inaktif from kegiatanrevisi as k inner join unitkerja as u on k.kodeuk=u.kodeuk" . $where;
    //echo $fsql;
    $countsql = "select count(*) as cnt from {kegiatanrevisi} k " . $where;
    //$fcountsql = sprintf($countsql, addslashes($nama));
	$fcountsql = $countsql;
    $result = pager_query($sql . $tablesort, $limit, 0, $fcountsql);
    ...
    if ($result) {
        while ($data = db_fetch_object($result)) {
			...
			if (user_access('kegiatanskpd edit')) {
				...
				$kegname = l($data->kegiatan, 'apbd/kegiatanrevisiperubahan/edit/' . $data->kodekeg , array('html' =>TRUE));
			} else {
			...
			if ($allowedit) {
				...
				$editlink =l('Rekening', 'apbd/kegiatanrevisiperubahan/rekening/' . $data->kodekeg, array('html'=>TRUE));
			}	
			if (isSuperuser()) {
				$editlink .= "&nbsp;" . l('Edit', 'apbd/kegiatanrevisiperubahan/editadmin/' . $data->kodekeg, array('html'=>TRUE));
			...
			*/
			...
			if (isSuperuser()) { 
				...
				if ($data->inaktif) 
					...
					$inaktif ='';
				*/
				$kegname = l($data->kegiatan, 'apbd/kegiatanrevisiperubahan/editadmin/' . $data->kodekeg, array('html'=>TRUE));
    ...
    $btn = "&nbsp;" . l("Daftar Kegiatan", 'apbd/kegiatanrevisiperubahan' , array ('html' => true, 'attributes'=> array ('class'=>'btn_blue', 'style'=>'color:white;')));
...
function GenDataViewBelumUsul() {
    ...
    $where = ' where k.totalp>k.plafon ' . $customwhere . $qlike . ' and k.kodekeg not in (select kodekeg from kegiatanrevisi) ';

	$sql = "select k.kodekeg,k.nomorkeg,k.tahun,k.kodeuk,k.kegiatan,k.lokasi,
			k.programtarget,k.totalp, k.plafon,u.namasingkat, k.adminok, k.sumberdana1 sumberdana, k.inaktif from kegiatanperubahan as k inner join unitkerja as u on k.kodeuk=u.kodeuk " . $where;
        ...
        while ($data = db_fetch_object($result)) {
			...
			if (isSuperuser()) { 
				...
				*/
				...
				$kegname = l($data->kegiatan, 'apbd/kegiatanrevisiperubahan/editadminbu/' . $data->kodekeg, array('html'=>TRUE));
...
function GenDataPrint() {
    ...
    $where = ' where k.totalp>k.plafon ' . $customwhere . $qlike ;

	$sql = "select k.kodekeg,k.nomorkeg,k.tahun,k.kodepro,k.kodeuk,k.kegiatan,k.lokasi,
			k.programtarget,k.totalp, k.plafon,u.namasingkat, k.adminok, k.sumberdana1 sumberdana, k.inaktif from kegiatanrevisi as k inner join unitkerja as u on k.kodeuk=u.kodeuk " . $where . " order by k.kegiatan, k.plafon";
	$result = db_query($sql);
	if ($result) {
		while ($data = db_fetch_object($result)) {
			$no += 1;
			...
			$rowsrek[] = array (
								 ...
								 array('data' => apbd_fn($data->totalp),  'width' => '90px', 'style' => ' border-right: 1px solid black; text-align:right;'),
								 );				
