	...
	if (isset($exportpdf) && ($exportpdf=='pdf'))  {
		if (isset($sampul))  {
			...
			if ($sampul=='sp') {
				...
				if ($tipedok=='rpka') {
					$pdfFile = 'matriks-permohonan-revisi-1-' . $id . '.pdf';
					...
					if ($isL) {
						$pdfFile = 'matriks-permohonan-revisi-2-' . $id . '.pdf';
						...
						$pdfFile = 'matriks-permohonan-revisi-3-' . $id . '.pdf';
			...
			} else if ($tipedok=='rpkasub') {
				$kodekeg = $id;	
				$pdfFile = 'rpka-skpd-perubahan-' . $kodekeg . '.pdf';
				$htmlContent = GenReportFormContentRKAsub($kodekeg);
...
function getDescription($id, $tipedok){
	...
	if ($tipedok=='rka') {
		...
		$rows[]= array (
					array('data' => '- Untuk mencetak RKA Perubahan, klik tombol RKA Perubahan', 'style' => 'border-none; text-align:left;'),
				 );							 
		$rows[]= array (
					array('data' => '- Untuk mencetak Surat Permohonan Perubahan, klik tombol Permohonan', 'style' => 'border-none; text-align:left;'),
				 );							 
		$rows[]= array (
					array('data' => '- Untuk mencetak Matrik Perubahan, klik tombol Matrik', 'style' => 'border-none; text-align:left;'),
				 );							 
		$rows[]= array (
						...
						$rowsrek[] = array (
							...
							if ($resultdetil) {
								while ($datadetil = db_fetch_object($resultdetil)) {
									$statusdetil = '';
									if ($penrekening > 0) {
										$sql = 'select iddetil,uraian,unitjumlah,unitsatuan,volumjumlah,volumsatuan,harga,total, pengelompokan,anggaran from {anggperkegdetil} where kodekeg=\'%s\' and kodero=\'%s\' and iddetil=\'%s\'';
										...
										if ($alldetilbintang) {
											...
											if (($datadetilpen->anggaran==0) and ($datadetilpen->total>0)) {
												$statusdetil = '<font color="Red">*</font>';
									...
									if ($datadetil->uraian == $datadetilpenuraian) {
										...
										if ($datadetil->pengelompokan) {
											//SUB DETIL
											$sql = 'select idsub,uraian,unitjumlah,unitsatuan,volumjumlah,volumsatuan, harga,total,anggaran from {anggperkegdetilsubrevisi} where iddetil=\'%s\' order by nourut asc,idsub';
											$fsql = sprintf($sql, db_escape_string($datadetil->iddetil));
											////////drupal_set_message($fsql);
											...
											if ($resultsub) {
												while ($datasub = db_fetch_object($resultsub)) {
													...
													if (($penrekening > 0) and ($datadetil->uraian == $datadetilpenuraian)) {
														...
														$sql = 'select idsub,uraian,unitjumlah,unitsatuan,volumjumlah,volumsatuan, harga,total,anggaran from {anggperkegdetilsub} where iddetil=\'%s\' and idsub=\'%s\'';
														...
														if ($resultsubpen) {
															if ($datasubpen = db_fetch_object($resultsubpen)) {
																...
																if ($statusdetil=='')
																	if ($datasubpen->total==$datasubpen->anggaran)
																		$statusdetilsub = '';
																	else
			...
			if ($resultobyek) {
				while ($dataobyek = db_fetch_object($resultobyek)) {
					...
					if ($result) {
						...
						$sql = 'select kodero,uraian,jumlah,anggaran from {anggperkeg} where kodekeg=\'%s\' and kodero=\'%s\'';
						$fsql = sprintf($sql, db_escape_string($kodekeg), db_escape_string($data->kodero));
						$resultpen = db_query($fsql);
								...
								while ($datadetil = db_fetch_object($resultdetil)) {
									...
									if ($penrekening > 0) {
										...
										if ($datadetilpen = db_fetch_object($resultdetilpen)) {
											$datadetilpenuraian = $datadetilpen->uraian;
											$datadetilpentotal = $datadetilpen->total;
											$datadetilpenetapanbintang = $datadetilpen->bintang;
										}
										
										//bintang penetapan
										if ($datadetilpenetapanbintang=='1') {
...
function GenReportFormContentRKAsub($kodekeg) {
	...
	*/
	...
	$headersrek[] = array (
	 ...
	 //JENIS
	$where = ' where k.kodekeg=\'%s\'';
	$sql = 'select mid(k.kodero,1,3) kodej,j.uraian,sum(jumlah) jumlahx from {anggperkegrevisi_0905} k  left join {jenis} j on mid(k.kodero,1,3)=j.kodej ' . $where;
	$fsql = sprintf($sql, db_escape_string($kodekeg));
	$fsql .= ' group by mid(k.kodero,1,3),j.uraian order by mid(k.kodero,1,3)';
	...
	if ($resultjenis) {
		while ($datajenis = db_fetch_object($resultjenis)) {
			...
			//OBYEK
			$sql = 'select mid(k.kodero,1,5) kodeo,o.uraian,sum(jumlah) jumlahx from {anggperkegrevisi_0905} k  left join {obyek} o on mid(k.kodero,1,5)=o.kodeo 
				   where kodekeg=\'%s\' and mid(k.kodero,1,3)=\'%s\'';
			$fsql = sprintf($sql, db_escape_string($kodekeg), db_escape_string($datajenis->kodej));
			$fsql .= ' group by mid(k.kodero,1,5),o.uraian order by mid(k.kodero,1,5)';
			...
			if ($resultobyek) {
				while ($dataobyek = db_fetch_object($resultobyek)) {
					...
					//REKENING
					$sql = 'select kodero,uraian,jumlah,anggaran,bintang from {anggperkegrevisi_0905} k where kodekeg=\'%s\' and mid(k.kodero,1,5)=\'%s\'';
					$fsql = sprintf($sql, db_escape_string($kodekeg), db_escape_string($dataobyek->kodeo));
					...
					if ($result) {
						...
						$rowsrek[] = array (
							...
							//DETIL
							$sql = 'select distinct iddetil,uraian,unitjumlah,unitsatuan,volumjumlah,volumsatuan,harga,total, pengelompokan, anggaran,bintang from {anggperkegdetilrevisi_0905} where kodekeg=\'%s\' and kodero=\'%s\' order by nourut asc,iddetil';
							$fsql = sprintf($sql, db_escape_string($kodekeg), db_escape_string($data->kodero));
							////////drupal_set_message($fsql);
							...
							if ($resultdetil) {
								while ($datadetil = db_fetch_object($resultdetil)) {
									$statusdetil = '';
									if ($penrekening > 0) {
										$sql = 'select iddetil,uraian,unitjumlah,unitsatuan,volumjumlah,volumsatuan,harga,total, pengelompokan,anggaran,bintang from {anggperkegdetil_0905} where kodekeg=\'%s\' and kodero=\'%s\' and iddetil=\'%s\'';
										$fsql = sprintf($sql, $kodekeg, $data->kodero, db_escape_string($datadetil->iddetil));
									...
									if ($datadetil->uraian == $datadetilpenuraian) {
										...
										if ($datadetil->pengelompokan) {
											//SUB DETIL
											$sql = 'select idsub,uraian,unitjumlah,unitsatuan,volumjumlah,volumsatuan, harga,total,anggaran,bintang from {anggperkegdetilsubrevisi_0905} where iddetil=\'%s\' order by nourut asc,idsub';
											$fsql = sprintf($sql, db_escape_string($datadetil->iddetil));
											////////drupal_set_message($fsql);
											...
											if ($resultsub) {
												while ($datasub = db_fetch_object($resultsub)) {
													...
													if (($penrekening > 0) and ($datadetil->uraian == $datadetilpenuraian)) {
														...
														$statusdetilsub_pen = '';
														$sql = 'select idsub,uraian,unitjumlah,unitsatuan,volumjumlah,volumsatuan, harga,total,anggaran,bintang from {anggperkegdetilsub_0905} where iddetil=\'%s\' and idsub=\'%s\'';
														$fsql = sprintf($sql, db_escape_string($datadetil->iddetil), db_escape_string($datasub->idsub));
														$resultsubpen = db_query($fsql);
														if ($resultsubpen) {
															if ($datasubpen = db_fetch_object($resultsubpen)) {
										...
										if (($datadetilpenuraian) !='' and ($datadetilpentotal>0)) {
											...
											if ($datadetil->pengelompokan) {
												...
												//SUB DETIL
												$sql = 'select idsub,uraian,unitjumlah,unitsatuan,volumjumlah,volumsatuan, harga,total,anggaran from {anggperkegdetilsub_0905} where iddetil=\'%s\' order by nourut asc,idsub';
												$fsql = sprintf($sql, db_escape_string($datadetil->iddetil));
										...
										if (($datadetil->uraian) !='' and ($datadetil->total>0)) {
											...
											if ($datadetil->pengelompokan) {
												...
												//SUB DETIL
												$sql = 'select idsub,uraian,unitjumlah,unitsatuan,volumjumlah,volumsatuan, harga,total,anggaran from {anggperkegdetilsubrevisi_0905} where iddetil=\'%s\' order by nourut asc,idsub';
												$fsql = sprintf($sql, db_escape_string($datadetil->iddetil));
		...
		while ($datajenis = db_fetch_object($resultjenis)) {
			...
			if ($resultobyek) {
				while ($dataobyek = db_fetch_object($resultobyek)) {
					...
					//REKENING
					$sql = 'select kodero,uraian,jumlah,anggaran from {anggperkegperubahan} k where kodekeg=\'%s\' and mid(k.kodero,1,5)=\'%s\'';
					$fsql = sprintf($sql, db_escape_string($kodekeg), db_escape_string($dataobyek->kodeo));
					...
					if ($result) {
						...
						$rowsrek[] = array (
							...
							//DETIL
							$sql = 'select distinct iddetil,uraian,unitjumlah,unitsatuan,volumjumlah,volumsatuan,harga,total, pengelompokan,anggaran from {anggperkegdetilperubahan} where kodekeg=\'%s\' and kodero=\'%s\' order by nourut asc,iddetil';
							$fsql = sprintf($sql, db_escape_string($kodekeg), db_escape_string($data->kodero));
							////////drupal_set_message($fsql);
...
function PrintSuratPermohonan($id) {
	...
	$sql = sprintf('select blno dpano,bltgl dpatgl from {dpanomor1} where kodeuk=\'%s\'' , db_escape_string($kodeuk));
	$res = db_query($sql);
	if ($data = db_fetch_object($res)) {
		$dpano = $data->dpano;
		$dpatgl = $data->dpatgl;
	...
	$rows[] = array (
				...
				array('data' => 'Alasan/pertimbangan perlunya Perubahan Anggaran:', 'colspan'=>'4', 'width' => '515px', 'style' => 'text-align:left;'),
				);
	$rows[] = array (
				array('data' => '',  'width'=> '20px', 'style' => 'text-align:left;'),
				array('data' => 'a.',  'width'=> '20px', 'style' => 'text-align:left;'),
...
function kegiatanskpd_printusulan_form () {
	$form['formdata'] = array (
		'#type' => 'fieldset',
		'#title'=> 'Dokumen Perubahan dan Setting Printer',
		'#collapsible' => FALSE,
		'#collapsed' => FALSE,        
	);
...
function PrintMatriksSPTJM($id) {
	...
	if ($jenisrevisi=='9') {
		...
		$str_revisi = 'revisi';
		$str_hukum = 'Peraturan Bupati Nomor 6 Tahun 2014 tentang Tata Cara Perubahan Anggaran';
	}
...
function PrintPersetujuan($id) {
	...
	*/
	...
	$rows[] = array (
				array('data' => 'Perihal',  'width'=> '50px', 'style' => 'text-align:left;'),
				array('data' => ':',  'width'=> '15px', 'style' => 'text-align:left;'),
				array('data' => 'Persetujuan Perubahan Anggaran', 'colspan'=>'3', 'width' => '460px', 'style' => 'text-align:left;'),
				);
	...
	switch($jenisrevisi) {
		case '1':		//GESER
			...
			//1
			$isian = 'Menunjuk Surat Saudara Nomor ' . $nosurat . ' Perihal Usulan Perubahan Anggaran tanggal ' . $tglsurat . ' dan berdasarkan Peraturan Bupati Jepara Nomor 6 Tahun 2014 Tentang Tata Cara Perubahan Anggaran, dapat kami sampaikan hal-hal sebagai berikut :';
			$rows[] = array (
						array('data' => $isian, 'colspan'=>'5', 'width' => '535px', 'style' => 'text-align:left;'),
						);
			
			$isian = 'Permohonan Saudara untuk melaksanakan perubahan atau pergeseran dalam hal pagu anggaran tetap untuk jenis ' . $ralat . ' kegiatan ' . $kegiatan . ' dapat kami setujui, hal ini sesuai dengan Pasal 3 Peraturan Bupati Jepara Nomor 6 Tahun 2014 Tentang Tata Cara Perubahan Anggaran.';
			$rows[] = array (
						array('data' => '1.',  'width'=> '20px', 'style' => 'text-align:left;'),
		...
		case '2':		//ADM
			...
			$isian = 'Permohonan Saudara untuk melaksanakan Perubahan/ralat karena kesalahan administrasi untuk Ralat/Kesalahan ' . $ralat . ' Kegiatan ' . $kegiatan . ' dapat kami setujui, hal ini sesuai dengan Pasal 4 Peraturan Bupati Jepara Nomor 6 Tahun 2014 Tentang Tata Cara Perubahan Anggaran';
			$rows[] = array (
						array('data' => '1.',  'width'=> '20px', 'style' => 'text-align:left;'),
		...
		case '3':		//DAK
			...
			$isian = 'Permohonan Saudara untuk melaksanakan Penambahan, pengurangan dan/atau pagu angggaran tetap yang dananya bersumber dari transfer Pemerintah Provinsi dan/atau Pemerintah kegiatan ' . $kegiatan . ' dapat kami setujui, hal ini sesuai dengan Pasal 5 Peraturan Bupati Jepara Nomor 6 Tahun 2014 Tentang Tata Cara Perubahan Anggaran.';
			$rows[] = array (
						array('data' => '1.',  'width'=> '20px', 'style' => 'text-align:left;'),
						array('data' => $isian, 'colspan'=>'3', 'width' => '515px', 'style' => 'text-align:left;'),
						);
...
function PrintPenolakan($id) {
	...
	*/
	...
	$rows[] = array (
				array('data' => 'Perihal',  'width'=> '50px', 'style' => 'text-align:left;'),
				array('data' => ':',  'width'=> '15px', 'style' => 'text-align:left;'),
				array('data' => 'Penolakan Perubahan Anggaran', 'colspan'=>'3', 'width' => '460px', 'style' => 'text-align:left;'),
				);
	...
	switch($jenisrevisi) {
		case '1':		//GESER
			...
			$isian = 'Permohonan Saudara untuk melaksanakan perubahan atau pergeseran dalam hal pagu anggaran tetap untuk jenis ' . $ralat . ' kegiatan ' . $kegiatan . '  kami tolak karena tidak sesuai dengan Pasal 3 Peraturan Bupati Jepara Nomor 6 Tahun 2014 Tentang Tata Cara Perubahan Anggaran ('.$jawaban.')';
			$rows[] = array (
						array('data' => '1.',  'width'=> '20px', 'style' => 'text-align:left;'),
						array('data' => $isian, 'colspan'=>'3', 'width' => '515px', 'style' => 'text-align:left;'),
						);
		...
		case '2':		//ADM
			...
			$isian = 'Permohonan Saudara untuk melaksanakan Perubahan/ralat karena kesalahan administrasi untuk Ralat/Kesalahan ' . $ralat . ' Kegiatan ' . $kegiatan . '  kami tolak karena tidak sesuai dengan Pasal 4 Peraturan Bupati Jepara Nomor 6 Tahun 2014 Tentang Tata Cara Perubahan Anggaran('.$jawaban.')';
			$rows[] = array (
						array('data' => '1.',  'width'=> '20px', 'style' => 'text-align:left;'),
						array('data' => $isian, 'colspan'=>'4', 'width' => '515px', 'style' => 'text-align:left;'),
						);
		...
		case '3':		//DAK
			...
			$isian = 'Permohonan Saudara untuk melaksanakan Penambahan, pengurangan dan/atau pagu angggaran tetap yang dananya bersumber dari transfer Pemerintah Provinsi dan/atau Pemerintah kegiatan ' . $kegiatan . '  kami tolak karena tidak sesuai dengan Pasal 4 Peraturan Bupati Jepara Nomor 6 Tahun 2014 Tentang Tata Cara Perubahan Anggaran('.$jawaban.')';
			$rows[] = array (
						array('data' => '1.',  'width'=> '20px', 'style' => 'text-align:left;'),
						array('data' => $isian, 'colspan'=>'3', 'width' => '515px', 'style' => 'text-align:left;'),
						);
