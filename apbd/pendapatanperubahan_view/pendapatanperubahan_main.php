<?php
function pendapatanperubahan_view_main($arg=NULL, $nama=NULL) {
        ...
        while ($data = db_fetch_object($result)) {
			...
			if ($allowedit) {
				if ($data->periode==$periodeaktif)
					$editlink =l('Hapus', 'apbd/pendapatanperubahan_view/delete/' . $data->kodeuk . '/'  . $data->kodero, array('html'=>TRUE));
				else
					$editlink = 'Hapus';
			}
            $no++;
...
function pendapatanperubahan_main_form_submit($form, &$form_state) {
	...
	$uri = 'apbd/pendapatanperubahan_view/filter/' . $kodeuk  . '/' . $kodek ;
	drupal_goto($uri);
