<?php 
function kegiatanrevisiperubahan_editadminbu_form() {
	...
	//if (isSuperuser()) $kodeuk = $_SESSION['kodeuk'];
...
function kegiatanrevisiperubahan_editadminbu_form_validate($form, &$form_state) {
...
function kegiatanrevisiperubahan_editadminbu_form_submit($form, &$form_state) {
    ...
    if($form_state['clicked_button']['#value'] == $form_state['values']['submitnext']) {
		...
		{					
			...
			$sql = sprintf("insert into {kegiatanperubahan} (kodekeg, nomorkeg, tahun, kodepro, kodeuk, kegiatan, jenis, plafon, sumberdana1, sumberdana1rp, sumberdana2, sumberdana2rp, inaktif, isppkd, dispensasi, periode) values('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')",
			...
			$sql = sprintf("update {kegiatanperubahan} set kegiatan='%s', plafon='%s', sumberdana1='%s', sumberdana1rp='%s', kodeuk='%s', kodepro='%s', jenis='%s', inaktif='%s',  isppkd='%s',
		...
		if ($res) {
			...
			if ($kodeberubah) {
				...
				$sql = sprintf("update {kegiatanrevisi} set kodekeg='%s',kodeuk='%s' where kodekeg='%s'",
				...
				$sql = sprintf("update {kegiatanrevisiperubahan} set kodekeg='%s',kodeuk='%s' where kodekeg='%s'",
								db_escape_string($kodekeg),
								db_escape_string($kodeuk),
								$e_kodekeg);	
				////drupal_set_message($sql);	
				...
				//Update anggperkegdetil
				/* TIDAK PERLU KARENA TIDAK ADA KODEKEG
