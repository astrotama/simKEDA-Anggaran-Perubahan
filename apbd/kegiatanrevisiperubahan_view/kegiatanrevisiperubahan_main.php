<?php
function kegiatanrevisiperubahan_view_main($arg=NULL, $nama=NULL) {
	/*
        ...
        while ($data = db_fetch_object($result)) {
			...
			*/
			...
			if ($data->total==0) {
				...
				$editlink .= l('Rek', 'apbd/kegiatanrevisiperubahan_view/rekening/' . $data->kodekeg  . '/0', array('html'=>TRUE));
				...
				if ($ada_revisi) {
					...
					if ($res_cek) {
						if ($data_cek = db_fetch_object($res_cek)) {
							if ($data_cek->x >0) {
								$editlink .= l('<font color="Chocolate">Rek</font>', 'apbd/kegiatanrevisiperubahan_view/rekening/' . $data->kodekeg  . '/0', array('html'=>TRUE));
...
function kegiatanrevisiperubahan_main_form_submit($form, &$form_state) {
	...
	$uri = 'apbd/kegiatanrevisiperubahan_view/filter/' . $kodeuk . '/' . $sumberdana . '/' . $statusisi . '/' . $kodesuk . '/'. $statustw . '/' . $statusinaktif . '/' . $jenis . '/' .$plafon . '/' . $jenisrevisi;
