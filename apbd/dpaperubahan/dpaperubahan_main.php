...
function dpaperubahan_main($arg=NULL, $nama=NULL) {
	...
	//JENIS REVISI
	if ($jenisrevisi == '1') {
		$qlike .= sprintf(' and k.totalp<>k.anggaran');
	}
    ...
    if ($result) {
        while ($data = db_fetch_object($result)) {
			...
			if ($data->jenis==1) {
				$tanggal = $data->btltgl;
				$dpanolengkap = str_replace('NNN',$data->btlno,$dpaformatbtl);
				$dpanolengkap = str_replace('NOKEG',$kodekeg,$dpanolengkap);
				
			} else {
				$tanggal = $data->bltgl;
				$dpanolengkap = str_replace('NNN',$data->blno,$dpaformatbl);
				$dpanolengkap = str_replace('NOKEG',$kodekeg,$dpanolengkap);
			}
			...
			if (isSuperuser())		//Superuser bisa mengubah TW
				$editlink = l('Triwulan', 'apbd/dpaperubahan/triwulan/' . $data->kodekeg, array('html'=>TRUE)) . "&nbsp;";
			...
			if (isSuperuser()) { 
				...
				$rows[] = array (
					...
					array('data' => $dpanolengkap . '<p  align="right">' . $tanggal . '</p>', 'align' => 'right', 'valign'=>'top'),
				...
				$rows[] = array (
					...
					array('data' => apbd_fn($data->totalp), 'align' => 'right', 'valign'=>'top'),
					array('data' => $tw, 'align' => 'right', 'valign'=>'top'),
					array('data' => $dpanolengkap . '<p>' . $tanggal . '</p>', 'align' => 'right', 'valign'=>'top'),
					array('data' => $ket, 'align' => 'left', 'valign'=>'top'),
					array('data' => $editlink, 'align' => 'left', 'valign'=>'top'),
...
function dpaperubahan_main_form() {
	...
	if (!isSuperuser()) {
		...
		//drupal_set_message('str ' . $strrevisi);
