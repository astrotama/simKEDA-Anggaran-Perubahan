<?php
function pembiayaanperubahan_view_main($arg=NULL, $nama=NULL) {
			...
			if ($allowedit) {
				if (user_access('kegiatanrkpd penghapusan')) {
					if ($data->periode==$periodeaktif)
						$editlink .=l('Hapus', 'apbd/pembiayaanperubahan_view/delete/' . $data->kodero, array('html'=>TRUE));
					else
						$editlink = 'Hapus';
				}
			}
...
function pembiayaanperubahan_main_form_submit($form, &$form_state) {
	...
	$uri = 'apbd/pembiayaanperubahan_view/filter/' . $kodek ;
	drupal_goto($uri);
