...
function GenReportForm($print=0,$revisi) {
	...
	*/
	...
	$sql = 'select p.kodepro,p.program,sum(totalp) jumlahx from {kegiatanperubahan} k left join {program} p 
	...
	if ($resultpro) {
		while ($datapro = db_fetch_object($resultpro)) {
			$totalp += $datapro->jumlahx;
			
			//rekap rekening
			$p_pegawai =0;
			$p_barangjasa =0;
			$p_modal = 0;
			$sql = 'select left(a.kodero,3) kodej, sum(a.jumlahp) jumlahx from {kegiatanperubahan} k left join {anggperkegperubahan} a on k.kodekeg=a.kodekeg where k.inaktif=0 and k.jenis=2 and k.kodeuk=\'%s\' and k.kodepro=\'%s\'';
			...
			//KEGIATAN
			$sql = 'select kodekeg,nomorkeg,lokasi,programtarget,kegiatan,totalp from {kegiatanperubahan} 
			...
			if ($resultkeg) {
				while ($datakeg = db_fetch_object($resultkeg)) {
					...
					$k_barangjasa =0;
					$k_modal = 0;
					$sql = 'select left(a.kodero,3) kodej, sum(a.jumlahp) jumlahx from {kegiatanperubahan} k left join {anggperkegperubahan} a on k.kodekeg=a.kodekeg where k.jenis=2 and k.inaktif=0 and k.kodeuk=\'%s\' and k.kodekeg=\'%s\'';
					...
					$rowsrek[] = array (
										 ...
										 array('data' => apbd_fn($datakeg->totalp),  'width' => '90px', 'style' => ' border-right: 1px solid black; text-align:right;'),
										 );		
	...
	*/
	...
	if ($resultpro) {
		while ($datapro = db_fetch_object($resultpro)) {
			...
			$p_barangjasa =0;
			$p_modal = 0;
			$sql = 'select left(a.kodero,3) kodej, sum(a.jumlahp) jumlahx from {kegiatanperubahan} k left join {anggperkegperubahan} a on k.kodekeg=a.kodekeg where k.jenis=2 and k.inaktif=0 and k.kodeuk=\'%s\' and k.kodepro=\'%s\'';
			$fsql = sprintf($sql, db_escape_string($kodeuk), db_escape_string($datapro->kodepro));
			$fsql .= ' group by left(a.kodero,3)';
