...
function GenReportForm($kodeuk, $jenis, $detil,$revisi) {
	...
	if ($result) {
		while ($data = db_fetch_object($result)) {
			...
			if ($jenis=='') {
				$sql = 'select sum(k.plafon) tplafon,sum(total) tanggaran from {kegiatanperubahan} k  where inaktif=0 and kodeuk=\'%s\' ';
				$fsql = sprintf($sql, db_escape_string($data->kodeuk));
			} else {
				$sql = 'select sum(plafon) tplafon,sum(total) tanggaran from {kegiatanperubahan} where inaktif=0 and kodeuk=\'%s\' and jenis=\'%s\'';
			...
			if ($detil==1) {
				//KEGIATAN
				if ($jenis=='') {
					$sql = 'select kegiatan, plafon, total, inaktif from {kegiatanperubahan} where kodeuk=\'%s\' ';
					$fsql = sprintf($sql, db_escape_string($data->kodeuk));
				} else {
					$sql = 'select kegiatan, plafon, total, inaktif from {kegiatanperubahan} where kodeuk=\'%s\' and jenis=\'%s\'';
					$fsql = sprintf($sql, db_escape_string($data->kodeuk), db_escape_string($jenis));
				}
...
function GenReportFormContent($kodeuk, $jenis, $detil,$revisi) {
	...
	if ($result) {
		while ($data = db_fetch_object($result)) {
			...
			if ($jenis=='') {
				$sql = 'select sum(plafon) tplafon,sum(total) tanggaran from {kegiatanperubahan} where inaktif=0 and kodeuk=\'%s\' ';
				$fsql = sprintf($sql, db_escape_string($data->kodeuk));
			} else {
				$sql = 'select sum(plafon) tplafon,sum(total) tanggaran from {kegiatanperubahan} where inaktif=0 and kodeuk=\'%s\' and jenis=\'%s\'';
				$fsql = sprintf($sql, db_escape_string($data->kodeuk), db_escape_string($jenis));
