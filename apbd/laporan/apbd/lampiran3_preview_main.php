<?php
function lampiran3_preview_main() {
		...
		while ($dataakun = db_fetch_object($resultakun)) {
			...
			if ($resultkel) {
				while ($datakel = db_fetch_object($resultkel)) {
					...
					//JENIS
					$sql = 'select j.kodej,j.uraian,sum(jumlah) jumlahx, sum(jumlahp) jumlahxp from {t_anggperkegrevisiproses} k inner join {jenis} j on mid(k.kodero,1,3)=j.kodej  inner join {t_kegiatanrevisisemua} g on k.kodekeg=g.kodekeg where g.inaktif=0 and g.kodeuk=\'%s\' and left(k.kodero,2)=\'%s\' and mid(k.kodero,1,2)=\'%s\'';
					$fsql = sprintf($sql, db_escape_string($kodeuk), db_escape_string($datakel->kodek), db_escape_string('51'));
					$fsql .= ' group by j.kodej,j.uraian order by j.kodej';
				...
				while ($datakel = db_fetch_object($resultkel)) {
					...
					if ($resultpro) {
						while ($datapro = db_fetch_object($resultpro)) {
							...
							//KEGIATAN
							$sql = 'select kodekeg, kegiatan uraian, total jumlahx , totalp jumlahxp from {kegiatanperubahan' . $str_table . '} k where k.inaktif=0 and k.kodeuk=\'%s\' and k.kodepro=\'%s\' and k.jenis=2';
							$fsql = sprintf($sql, db_escape_string($kodeuk), db_escape_string($datapro->kodepro));
							$fsql .= ' order by k.kodepro, k.kodekeg';			
							$resultkeg = db_query($fsql);
							if ($resultkeg) {
								while ($datakeg = db_fetch_object($resultkeg)) {
...
function lampiran3_form_submit($form, &$form_state) {
	...
	if($form_state['clicked_button']['#value'] == $form_state['values']['tampilkan']) 
        $uri = 'apbd/laporan/apbd/lampiran3preview/'. $revisi . '/'  . $kodeuk . '/' . $topmargin . '/' . $hal1 . '/' . $lampiran . '/' . $judul . '/'. $ttd ;
	else	
		$uri = 'apbd/laporan/apbd/lampiran3preview/' . $revisi. '/' . $kodeuk . '/' . $topmargin . '/' . $hal1 . '/' . $lampiran . '/' . $judul . '/'. $ttd . '/pdf' ;
