...
function GenReportFormContent($kodeuk, $revisi, $lampiran, $judul) {
	...
	*/
	...
	if ($kodeuk=='81') {
		...
		$sql = 'select x.kodek,x.uraian,sum(jumlah) jumlahx, sum(jumlahp) jumlahxp from {anggperdaperubahan} k inner join {kelompok} x on mid(k.kodero,1,2)=x.kodek group by x.kodek,x.uraian order by x.kodek';
