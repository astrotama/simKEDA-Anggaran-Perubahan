<?php
function perdabup_preview_main() {
...
drupal_add_css('files/css/kegiatancam.css');
	...
	if (isset($exportpdf) && ($exportpdf=='pdf'))  {
		...
		$output = drupal_get_form('perdabup_preview_form');
...
function GenReportFormContent($kodeuk, $perdabup, $lampiran,$revisi) {
	...
	$sql = 'select a.kodea,a.uraian,sum(k.jumlah) jumlahx,sum(k.jumlahp) jumlahxp from {t_anggperkegrevisiproses} k  inner join {anggaran} a on mid(k.kodero,1,1)=a.kodea inner join {t_kegiatanrevisisemua} g on k.kodekeg=g.kodekeg inner join {t_kegiatanrevisisemua} keg on k.kodekeg=keg.kodekeg where keg.inaktif=0 ' . $where;
	$fsql = sprintf($sql, db_escape_string($kodeuk));
	$fsql .= ' group by a.kodea,a.uraian order by a.kodea';
	...
	if ($resultakun) {
		while ($dataakun = db_fetch_object($resultakun)) {
			...
			//KELOMPOK - BTL
			$sql = 'select x.kodek,x.uraian,sum(jumlah) jumlahx,sum(jumlahp) jumlahxp from {t_anggperkegrevisiproses} k inner join {kelompok} x on mid(k.kodero,1,2)=x.kodek inner join {t_kegiatanrevisisemua} g on k.kodekeg=g.kodekeg where g.inaktif=0 and g.kodeuk=\'%s\' and left(k.kodero,2)=\'%s\' and mid(k.kodero,1,1)=\'%s\'';
			...
			if ($resultkel) {
				while ($datakel = db_fetch_object($resultkel)) {
					...
					//JENIS
					$sql = 'select j.kodej,j.uraian,sum(jumlah) jumlahx,sum(jumlahp) jumlahxp from {t_anggperkegrevisiproses} k inner join {jenis} j on mid(k.kodero,1,3)=j.kodej  inner join {t_kegiatanrevisisemua} g on k.kodekeg=g.kodekeg where g.inaktif=0 and g.kodeuk=\'%s\' and j.kodek=\'%s\'';
					$fsql = sprintf($sql, db_escape_string($kodeuk), db_escape_string($datakel->kodek));
					$fsql .= ' group by j.kodej,j.uraian order by j.kodej';
					...
					if ($resultj) {
						while ($dataj = db_fetch_object($resultj)) {
							...
							if ($perdabup == 'perbup') {
								//OBYEK
								$sql = 'select o.kodeo,o.uraian,sum(jumlah) jumlahx, sum(jumlahp) jumlahxp from {t_anggperkegrevisiproses} k inner join {obyek} o on mid(k.kodero,1,5)=o.kodeo inner join {t_kegiatanrevisisemua} g on k.kodekeg=g.kodekeg where g.inaktif=0 and g.kodeuk=\'%s\' and o.kodej=\'%s\'';
								$fsql = sprintf($sql, db_escape_string($kodeuk), db_escape_string($dataj->kodej));
								$fsql .= ' group by o.kodeo,o.uraian order by o.kodeo';
								...
								if ($resulto) {
									while ($datao = db_fetch_object($resulto)) {
										...
										//RINCIAN OBYEK
										$sql = 'select ro.kodero,ro.uraian,sum(jumlah) jumlahx,sum(jumlahp) jumlahxp from {t_anggperkegrevisiproses} k inner join {rincianobyek} ro on k.kodero=ro.kodero inner join {t_kegiatanrevisisemua} g on k.kodekeg=g.kodekeg where g.inaktif=0 and g.kodeuk=\'%s\' and ro.kodeo=\'%s\'';
										$fsql = sprintf($sql, db_escape_string($kodeuk), db_escape_string($datao->kodeo));
										...
										if ($result) {
											while ($dataro = db_fetch_object($result)) {
												...
												if (($dataj->kodej == '516') or ($dataj->kodej == '517') ) {
													//KEGIATAN
													$sql = 'select k.kodekeg,k.kegiatan,a.jumlah as jumlahx, a.jumlahp as jumlahxp from {t_kegiatanrevisisemua} k inner join {t_anggperkegrevisiproses} a on k.kodekeg=a.kodekeg where a.kodero=\'%s\' order by kegiatan';
			...
			//KELOMPOK - BL
			$sql = 'select x.kodek,x.uraian,sum(jumlah) jumlahx, sum(jumlahp) jumlahxp from {t_anggperkegrevisiproses} k inner join {kelompok} x on mid(k.kodero,1,2)=x.kodek inner join {t_kegiatanrevisisemua} g on k.kodekeg=g.kodekeg where g.inaktif=0 and g.kodeuk=\'%s\' and left(k.kodero,2)=\'%s\'';
			...
			if ($resultkel) {
				while ($datakel = db_fetch_object($resultkel)) {
					...
					//PROGRAM
					$sql = 'select k.kodepro, p.program uraian,sum(k.total) jumlahx, sum(k.totalp) jumlahxp from {t_kegiatanrevisisemua} k 
					...
					if ($resultpro) {
						while ($datapro = db_fetch_object($resultpro)) {
							...
							//KEGIATAN
							$sql = 'select kodekeg, kegiatan uraian,sumberdana1, total jumlahx, totalp jumlahxp,lokasi from {t_kegiatanrevisisemua} k where k.inaktif=0 and k.kodeuk=\'%s\' and k.kodepro=\'%s\' and k.jenis=2';
							...
							if ($resultkeg) {
								while ($datakeg = db_fetch_object($resultkeg)) {
									if (($datakeg->jumlahxp+$datakeg->jumlahx)>0) {
										...
										$sdindex =0; 
										//REK JENIS
										$sql = 'select j.kodej,j.uraian,sum(k.jumlah) jumlahx,sum(k.jumlahp) jumlahxp from {t_anggperkegrevisiproses} k inner join {jenis} j on mid(k.kodero,1,3)=j.kodej  inner join {t_kegiatanrevisisemua} g on k.kodekeg=g.kodekeg where g.kodeuk=\'%s\' and g.kodekeg=\'%s\'';
										...
										if ($resultj) {
											while ($dataj = db_fetch_object($resultj)) {
												...
												if ($perdabup == 'perbup') {
													//OBYEK
													$sql = 'select o.kodeo,o.uraian,sum(jumlah) jumlahx,sum(jumlahp) jumlahxp from {t_anggperkegrevisiproses} k inner join {obyek} o on mid(k.kodero,1,5)=o.kodeo inner join {t_kegiatanrevisisemua} g on k.kodekeg=g.kodekeg where g.kodeuk=\'%s\' and g.kodekeg=\'%s\' and o.kodej=\'%s\'';
													...
													if ($resulto) {
														while ($datao = db_fetch_object($resulto)) {
															...
															//RINCIAN OBYEK
															$sql = 'select ro.kodero,ro.uraian,jumlah jumlahx, jumlahp jumlahxp from {t_anggperkegrevisiproses} k inner join {rincianobyek} ro on k.kodero=ro.kodero inner join {t_kegiatanrevisisemua} g on k.kodekeg=g.kodekeg where g.kodeuk=\'%s\' and g.kodekeg=\'%s\' and ro.kodeo=\'%s\'';
...
function perdabup_preview_form () {
	$form['formdata'] = array (
		'#type' => 'fieldset',
...
function perdabup_preview_form_submit($form, &$form_state) {
	...
	if($form_state['clicked_button']['#value'] == $form_state['values']['tampilkan']) 
        $uri = 'apbd/laporan/apbd/perdabuppreview/'.$revisi.'/' . $kodeuk . '/' . $topmargin . '/' . $hal1 . '/' . $perdabup  . '/' . $lampiran . '/' . $judul . '/' . $ttd;
	else	
		$uri = 'apbd/laporan/apbd/perdabuppreview/'.$revisi.'/' . $kodeuk . '/' . $topmargin . '/' . $hal1 . '/' . $perdabup  . '/' . $lampiran . '/' . $judul . '/' . $ttd . '/pdf' ;
