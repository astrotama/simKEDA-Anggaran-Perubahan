										...
										if ($result) {
											while ($dataro = db_fetch_object($result)) {
												...
												if (($dataj->kodej == '516') or ($dataj->kodej == '517') ) {
													//KEGIATAN
													$sql = 'select k.kodekeg,k.kegiatan,a.jumlah as jumlahx, a.jumlahp as jumlahxp from {kegiatanperubahan' . $str_table . '} k inner join {anggperkegperubahan' . $str_table . '} a on k.kodekeg=a.kodekeg where k.inaktif=0 and (k.total+k.totalp)>0 and a.kodero=\'%s\' order by kegiatan';
