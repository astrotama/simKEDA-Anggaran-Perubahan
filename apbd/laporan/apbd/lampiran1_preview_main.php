<?php
function lampiran1_preview_main() {
...
drupal_add_css('files/css/kegiatancam.css');
	...
	*/
	...
	} elseif (isset($exportpdf) && ($exportpdf=='dbg'))  {
		$output = GenReportFormContent_Debug();
		return $output;
		
	} else {
		//$url = 'apbd/laporan/apbd/lampiran1/'. $kodeuk . '/' . $topmargin . '/' . $hal1 . '/pdf';
		
		$output = drupal_get_form('lampiran1_preview_form');
...
function GenReportFormContent($kodeuk, $tingkat) {
	...
	if ($kodeuk!='00') {
		...
		$sql = 'select a.kodea,a.uraian,sum(jumlah) jumlahx,sum(jumlahp) jumlahxp  from {anggperukperubahan} k  inner join {anggaran} a on mid(k.kodero,1,1)=a.kodea ' . $where;
		$fsql = sprintf($sql, db_escape_string($kodeuk));
		$fsql .= ' group by a.kodea,a.uraian order by a.kodea';
	
	} else {
		$sql = 'select mid(k.kodero,1,1) kodea,a.uraian,sum(jumlah) jumlahx,sum(jumlahp) jumlahxp  from {anggperukperubahan} k  inner join {anggaran} a on mid(k.kodero,1,1)=a.kodea ';
	...
	if ($resultakun) {
		while ($dataakun = db_fetch_object($resultakun)) {
			...
			//KELOMPOK
			if ($kodeuk!='00') {
				$sql = 'select x.kodek,x.uraian,sum(jumlah) jumlahx,sum(jumlahp) jumlahxp from {anggperukperubahan} k inner join {kelompok} x on mid(k.kodero,1,2)=x.kodek where kodeuk=\'%s\' and mid(k.kodero,1,1)=\'%s\'';
				$fsql = sprintf($sql, db_escape_string($kodeuk), db_escape_string($dataakun->kodea));
				$fsql .= ' group by x.kodek,x.uraian order by x.kodek';
				
			} else {
				$sql = 'select mid(k.kodero,1,2) kodek,x.uraian,sum(jumlah) jumlahx,sum(jumlahp) jumlahxp from {anggperukperubahan} k inner join {kelompok} x on mid(k.kodero,1,2)=x.kodek where mid(k.kodero,1,1)=\'%s\'';
			...
			if ($resultkel) {
				while ($datakel = db_fetch_object($resultkel)) {
					...
					//JENIS
					if ($kodeuk!='00') {
						$sql = 'select j.kodej,j.uraian,sum(jumlah) jumlahx ,sum(jumlahp) jumlahxp from {anggperukperubahan} k inner join {jenis} j on mid(k.kodero,1,3)=j.kodej where kodeuk=\'%s\' and mid(k.kodero,1,2)=\'%s\'';
						$fsql = sprintf($sql, db_escape_string($kodeuk), db_escape_string($datakel->kodek));
						$fsql .= ' group by j.kodej,j.uraian order by j.kodej';
						
					} else {
						$sql = 'select j.kodej,j.uraian,sum(jumlah) jumlahx ,sum(jumlahp) jumlahxp from {anggperukperubahan} k inner join {jenis} j on mid(k.kodero,1,3)=j.kodej where mid(k.kodero,1,2)=\'%s\'';
					...
					if ($result) {
						while ($data = db_fetch_object($result)) {
							...
							if ($tingkat>=4) {
								if ($kodeuk!='00') {
									$sql = 'select r.kodeo,r.uraian,sum(jumlah) jumlahx ,sum(jumlahp) jumlahxp from {anggperukperubahan} k inner join {obyek} r on mid(k.kodero,1,5)=r.kodeo where kodeuk=\'%s\' and mid(k.kodero,1,3)=\'%s\'';
									$fsql = sprintf($sql, db_escape_string($kodeuk), db_escape_string($data->kodej));
									$fsql .= ' group by r.kodeo,r.uraian order by r.kodeo';
									
								} else {
									$sql = 'select r.kodeo,r.uraian,sum(jumlah) jumlahx ,sum(jumlahp) jumlahxp from {anggperukperubahan} k inner join {obyek} r on mid(k.kodero,1,5)=r.kodeo where mid(k.kodero,1,3)=\'%s\'';
								...
								if ($resulto) {
									while ($datao = db_fetch_object($resulto)) {
										...
										if ($tingkat>=5) {
											if ($kodeuk!='00') {
												$sql = 'select r.kodero,r.uraian,sum(jumlah) jumlahx ,sum(jumlahp) jumlahxp from {anggperukperubahan} k inner join {rincianobyek} r on k.kodero=r.kodero where kodeuk=\'%s\' and mid(k.kodero,1,5)=\'%s\'';
												$fsql = sprintf($sql, db_escape_string($kodeuk), db_escape_string($datao->kodeo));
												$fsql .= ' order by r.kodero';
												
											} else {
												$sql = 'select r.kodero,r.uraian,sum(jumlah) jumlahx ,sum(jumlahp) jumlahxp from {anggperukperubahan} k inner join {rincianobyek} r on k.kodero=r.kodero where mid(k.kodero,1,5)=\'%s\'';
	...
	if ($kodeuk!='00') {
		...
		$sql = 'select a.kodea,a.uraian,sum(jumlah) jumlahx, sum(jumlahp) jumlahxp from {t_anggperkegrevisiproses} k  inner join {anggaran} a on mid(k.kodero,1,1)=a.kodea inner join {t_kegiatanrevisisemua} g on k.kodekeg=g.kodekeg where g.inaktif=0 ' . $where;
		$fsql = sprintf($sql, db_escape_string($kodeuk));
		$fsql .= ' group by a.kodea,a.uraian order by a.kodea';
	
	} else {
		$sql = 'select a.kodea,a.uraian,sum(jumlah) jumlahx, sum(jumlahp) jumlahxp from {t_anggperkegrevisiproses} k  inner join {anggaran} a on mid(k.kodero,1,1)=a.kodea inner join {t_kegiatanrevisisemua} g on k.kodekeg=g.kodekeg ';
	...
	if ($resultakun) {
		while ($dataakun = db_fetch_object($resultakun)) {
			...
			//KELOMPOK
			if ($kodeuk!='00') {
				$sql = 'select x.kodek,x.uraian,sum(jumlah) jumlahx, sum(jumlahp) jumlahxp from {t_anggperkegrevisiproses} k inner join {kelompok} x on mid(k.kodero,1,2)=x.kodek inner join {t_kegiatanrevisisemua} g on k.kodekeg=g.kodekeg where g.inaktif=0 and g.kodeuk=\'%s\' and mid(k.kodero,1,1)=\'%s\'';
				$fsql = sprintf($sql, db_escape_string($kodeuk), db_escape_string($dataakun->kodea));
				$fsql .= ' group by x.kodek,x.uraian order by x.kodek';
			
			} else {
				$sql = 'select x.kodek,x.uraian,sum(jumlah) jumlahx, sum(jumlahp) jumlahxp from {t_anggperkegrevisiproses} k inner join {kelompok} x on mid(k.kodero,1,2)=x.kodek inner join {t_kegiatanrevisisemua} g on k.kodekeg=g.kodekeg where g.inaktif=0 and mid(k.kodero,1,1)=\'%s\'';
				$fsql = sprintf($sql, db_escape_string($dataakun->kodea));
				$fsql .= ' group by x.kodek,x.uraian order by  x.kodek';
			}
			//drupal_set_message( $fsql);
			...
			if ($resultkel) {
				while ($datakel = db_fetch_object($resultkel)) {
					...
					//JENIS
					if ($kodeuk!='00') { 
						$sql = 'select j.kodej,j.uraian,sum(jumlah) jumlahx, sum(jumlahp) jumlahxp from {t_anggperkegrevisiproses} k inner join {jenis} j on mid(k.kodero,1,3)=j.kodej  inner join {t_kegiatanrevisisemua} g on k.kodekeg=g.kodekeg where g.inaktif=0 and g.kodeuk=\'%s\' and mid(k.kodero,1,2)=\'%s\'';
						$fsql = sprintf($sql, db_escape_string($kodeuk), db_escape_string($datakel->kodek));
						$fsql .= ' group by j.kodej,j.uraian order by j.kodej';
					
					} else {
						$sql = 'select j.kodej,j.uraian,sum(jumlah) jumlahx, sum(jumlahp) jumlahxp from {t_anggperkegrevisiproses} k inner join {jenis} j on mid(k.kodero,1,3)=j.kodej  inner join {t_kegiatanrevisisemua} g on k.kodekeg=g.kodekeg where g.inaktif=0 and mid(k.kodero,1,2)=\'%s\'';
						$fsql = sprintf($sql, db_escape_string($datakel->kodek));
					...
					if ($result) {
						while ($data = db_fetch_object($result)) {
							...
							if ($tingkat>=4) {
								if ($kodeuk!='00') { 
									$sql = 'select r.kodeo,r.uraian,sum(jumlah) jumlahx, sum(jumlahp) jumlahxp from {t_anggperkegrevisiproses} k inner join {obyek} r on mid(k.kodero,1,5)=r.kodeo inner join {t_kegiatanrevisisemua} g on k.kodekeg=g.kodekeg where g.inaktif=0 and g.kodeuk=\'%s\' and mid(k.kodero,1,3)=\'%s\'';
									$fsql = sprintf($sql, db_escape_string($kodeuk), db_escape_string($data->kodej));
									$fsql .= ' group by r.kodeo,r.uraian order by r.kodeo';
								
								} else {
									$sql = 'select r.kodeo,r.uraian,sum(jumlah) jumlahx, sum(jumlahp) jumlahxp from {t_anggperkegrevisiproses} k inner join {obyek} r on mid(k.kodero,1,5)=r.kodeo inner join {t_kegiatanrevisisemua} g on k.kodekeg=g.kodekeg where g.inaktif=0 and mid(k.kodero,1,3)=\'%s\'';
									$fsql = sprintf($sql, db_escape_string($data->kodej));
									$fsql .= ' group by r.kodeo,r.uraian order by r.kodeo';
								}
								...
								if ($resulto) {
									while ($datao = db_fetch_object($resulto)) {
										...
										if ($tingkat>=5) {
											if ($kodeuk!='00') { 
												$sql = 'select r.kodero,r.uraian,sum(jumlah) jumlahx, sum(jumlahp) jumlahxp from {t_anggperkegrevisiproses} k inner join {rincianobyek} r on k.kodero=r.kodero inner join {t_kegiatanrevisisemua} g on k.kodekeg=g.kodekeg where g.inaktif=0 and g.kodeuk=\'%s\' and mid(k.kodero,1,5)=\'%s\'';
												$fsql = sprintf($sql, db_escape_string($kodeuk), db_escape_string($datao->kodeo));
												$fsql .= ' group by r.kodero,r.uraian order by r.kodero';
											
											} else {
												$sql = 'select r.kodero,r.uraian,sum(jumlah) jumlahx, sum(jumlahp) jumlahxp from {t_anggperkegrevisiproses} k inner join {rincianobyek} r on k.kodero=r.kodero inner join {t_kegiatanrevisisemua} g on k.kodekeg=g.kodekeg where g.inaktif=0 and mid(k.kodero,1,5)=\'%s\'';
												$fsql = sprintf($sql, db_escape_string($datao->kodeo));
												$fsql .= ' group by r.kodero,r.uraian order by r.kodero';
											} 
											//drupal_set_message($fsql);
...
function GenReportFormContent_Debug() {
	$total = 0;
	$totalp = 0;
	
	$sql = 'select distinct a.kodekeg, a.kodero, a.jumlah from anggperkegperubahan1 as a inner join kegiatanperubahan1 as k on a.kodekeg=k.kodekeg where k.inaktif=0 order by a.kodekeg, a.kodero';
	$res_rev = db_query($sql);
	if ($res_rev) { 
		while ($data_rev = db_fetch_object($res_rev)) {
			$jumlahp = 0;
			$total += $data_rev->jumlah;
			//drupal_set_message($data_rev->kodekeg . ', ' . $data_rev->kodero);
			
			$sql = sprintf("select a.kodekeg, a.kodero, a.jumlah from anggperkegperubahan as a inner join kegiatanperubahan as k on a.kodekeg=k.kodekeg where a.kodekeg='%s' and a.kodero='%s'", $data_rev->kodekeg, $data_rev->kodero) ;
			//drupal_set_message($sql);
			$res_cek = db_query($sql);
			if ($res_cek) {
				while ($data_cek = db_fetch_object($res_cek)) {
					
					$jumlahp = $data_cek->jumlah;
					
				} 
			} else {
					$rowsfooter[] = array (
										 array('data' => $data_rev->kodekeg,  'width'=> '335px',  'colspan'=>'2',  'style' => 'text-align:center'),
										 array('data' => $data_rev->kodero,  'width' => '375px', 'style' => 'text-align:center;'),
										 array('data' => 'yy',  'width' => '100px', 'style' => 'text-align:center;'),
										 );
			}
			
			$totalp += $jumlahp;
			if ($jumlahp != $data_rev->jumlah) {
				$rowsfooter[] = array (
									 array('data' => $data_rev->kodekeg,  'width'=> '335px',  'colspan'=>'2',  'style' => 'text-align:center'),
									 array('data' => $data_rev->kodero,  'width' => '175px', 'style' => 'text-align:center;'),
									 array('data' => 'aa',  'width' => '100px', 'style' => 'text-align:center;'),
									 array('data' => $data_rev->jumlah,  'width' => '175px', 'style' => 'text-align:center;'),
									 );
			}			
	...
	$rowsfooter[] = array (
						 array('data' => 'Total',  'width'=> '335px',  'colspan'=>'2',  'style' => 'text-align:center'),
						 array('data' => $total,  'width' => '375px', 'style' => 'text-align:center;'),
						 array('data' => $totalp,  'width' => '100px', 'style' => 'text-align:center;'),
						 );	
...
function lampiran1_preview_form () {
	$form['formdata'] = array (
		'#type' => 'fieldset',
...
function lampiran1_preview_form_submit($form, &$form_state) {
	...
	if($form_state['clicked_button']['#value'] == $form_state['values']['tampilkan']) {
		...
		$uri = 'apbd/laporan/apbd/lampiran1preview/' . $revisi . '/' . $kodeuk . '/' . $tingkat . '/'. $topmargin . '/' . $hal1;
		drupal_goto($uri);
		...
		$uri = 'apbd/laporan/apbd/lampiran1preview/' . $revisi . '/' . $kodeuk . '/' . $tingkat . '/'. $topmargin . '/' . $hal1 . '/pdf' ;
		drupal_goto($uri);
	} else {
		doRekap();
	}
...
function doRekap() {
	...
	/*
	UPDATE anggperkeg INNER JOIN anggperkeg_77
	ON anggperkeg.kodekeg=anggperkeg_77.kodekeg AND anggperkeg.kodero=anggperkeg_77.kodero
	SET anggperkeg.jumlah=anggperkeg_77.jumlah

	UPDATE anggperkegperubahan INNER JOIN anggperkeg_77
	ON anggperkegperubahan.kodekeg=anggperkeg_77.kodekeg AND anggperkegperubahan.kodero=anggperkeg_77.kodero
	SET anggperkegperubahan.jumlah=anggperkeg_77.jumlah

	UPDATE anggperkegrevisi INNER JOIN anggperkeg_77
	ON anggperkegrevisi.kodekeg=anggperkeg_77.kodekeg AND anggperkegrevisi.kodero=anggperkeg_77.kodero
	SET anggperkegrevisi.jumlahpenetapan=anggperkeg_77.jumlah	
	*/
	
	$res = db_query('update anggperkegrevisi inner join anggperkeg on anggperkegrevisi.kodekeg=anggperkeg.kodekeg and anggperkegrevisi.kodero=anggperkeg.kodero set anggperkegrevisi.jumlahpenetapan=anggperkeg.jumlah');
	$res = db_query('delete from t_kegiatanrevisisemua');
	$res = db_query('delete from t_anggperkegrevisiproses');

	$res = db_query('insert into t_kegiatanrevisisemua select * from q_kegiatanrevisisemua');
	$res = db_query('insert into t_anggperkegrevisiproses select * from q_anggperkegrevisiproses');
