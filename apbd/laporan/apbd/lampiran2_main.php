...
function GenReportFormContent($revisi) {
	...
	for ($u=1; $u<=4; $u++) {
		...
		//Belanja
		$sql = 'select k.jenis, sum(k.penetapan) as jumlahx,sum(k.anggaran) as jumlahxp from {t_kegiatanrevisisemua} k inner join {unitkerja} u on
		...
		if ($u==1)
			$ju = 'URUSAN WAJIB';
		elseif ($u==2)
			$ju = 'URUSAN PILIHAN PELAYANAN DASAR';
		elseif ($u==3)
			$ju = 'URUSAN PILIHAN NON PELAYANAN DASAR';
		else
			$ju = 'URUSAN PENDUKUNG PEMERINTAHAN';
		...
		if ($resultu) {
			while ($datau = db_fetch_object($resultu)) {
				...
				//PENDAPATAN
				$sql = 'select sum(k.jumlah) jumlahx, sum(k.jumlahp) jumlahxp from {anggperukperubahan} k inner join {unitkerja} u on
				...
				//Belanja
				$sql = 'select k.jenis, sum(k.penetapan) as jumlahx, sum(k.anggaran) jumlahxp from {t_kegiatanrevisisemua} k inner join {unitkerja} u on
				...
				if ($resultuk) {
					while ($datauk = db_fetch_object($resultuk)) {
						...
						//PENDAPATAN
						$sql = 'select sum(jumlah) jumlahx, sum(jumlahp) jumlahxp from {anggperukperubahan} ' . $whereuk;
						$res = db_query($sql);	
						if ($res) 	{
							if ($data = db_fetch_object($res)) {
						...
						//Belanja
						$sql = 'select jenis, sum(penetapan) as jumlahx,  sum(anggaran) as jumlahxp from {t_kegiatanrevisisemua} ' . $whereuk . ' and inaktif=0 group by jenis';
						//drupal_set_message($sql);
						$res = db_query($sql);
						if ($res) 	{
							while ($data = db_fetch_object($res)) {
...
function GenReportFormContent_Asli($revisi) {
