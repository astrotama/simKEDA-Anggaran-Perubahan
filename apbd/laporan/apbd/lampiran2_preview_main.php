...
function lampiran2_preview_main() {
	...
	for ($u=1; $u<=4; $u++) {
		...
		if ($resultu) {
			while ($datau = db_fetch_object($resultu)) {
				...
				//PENDAPATAN
				$sql = 'select sum(k.jumlah) jumlahx, sum(k.jumlahp) jumlahxp from {t_kegiatanrevisisemua} k inner join {unitkerja} u on
				...
				if ($resultuk) {
					while ($datauk = db_fetch_object($resultuk)) {
						...
						//PENDAPATAN
						$sql = 'select sum(jumlah) jumlahx, sum(jumlahp) jumlahxp from {t_kegiatanrevisisemua} ' . $whereuk;
						$res = db_query($sql);	
						if ($res) 	{
							if ($data = db_fetch_object($res)) {
