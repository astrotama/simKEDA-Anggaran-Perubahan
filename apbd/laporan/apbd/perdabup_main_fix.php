										...
										if ($result) {
											while ($dataro = db_fetch_object($result)) {
												...
												if (($dataj->kodej == 516) or ($dataj->kodej == 517) ) {
													//KEGIATAN
													$sql = 'select k.kodekeg,k.kegiatan,a.jumlah from {kegiatanskpd} k inner join {anggperkeg} a on k.kodekeg=a.kodekeg where a.inaktif=0 and a.kodero=\'%s\' order by kegiatan';
