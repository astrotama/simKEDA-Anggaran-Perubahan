<?php
function lampiran5_preview_main() {
	...
	if ($resultf) 	{
		while ($dataf = db_fetch_object($resultf)) {
			...
			$sql = 'select left(a.kodero,3) kode, sum(a.jumlah) as anggaran, sum(a.jumlahp) as anggaranp from {t_anggperkegrevisiproses} a inner join 
					{t_kegiatanrevisisemua} k on a.kodekeg=k.kodekeg inner join {program} p on k.kodepro=p.kodepro inner join {urusan} u on p.kodeu=u.kodeu ' . $where . ' and k.inaktif=0 group by left(a.kodero,3)';
			//drupal_set_message($sql);
			$res = db_query($sql);	
			if ($res) 	{
				while ($data = db_fetch_object($res)) {
			...
			if ($resultu) {
				while ($datau = db_fetch_object($resultu)) {
					...
					//Belanja
					$sql = 'select left(a.kodero,3) kode, sum(a.jumlah) as anggaran, sum(a.jumlahp) as anggaranp from {t_anggperkegrevisiproses} a inner join 
							{t_kegiatanrevisisemua} k on a.kodekeg=k.kodekeg inner join {program} p on k.kodepro=p.kodepro ' . $where . ' and k.inaktif=0 group by left(a.kodero,3)';
					$res = db_query($sql);	
					if ($res) 	{
						while ($data = db_fetch_object($res)) {
							switch($data->kode) {
...
function lampiran5_form_submit($form, &$form_state) {
	...
	if($form_state['clicked_button']['#value'] == $form_state['values']['tampilkan']) 
		$uri = 'apbd/laporan/apbd/lampiran5preview/'.$revisi.'/' . $topmargin . '/' . $hal1 ;
	else
		$uri = 'apbd/laporan/apbd/lampiran5preview/'.$revisi.'/' . $topmargin . '/' . $hal1 . '/pdf' ;
	drupal_goto($uri);
