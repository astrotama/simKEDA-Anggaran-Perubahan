...
function getTglDPA() {
	$sql = 'select revisi, dpatgl1, dpatgl2, dpatgl3, dpatgl4, dpatgl5 
	from {setupapp}';
	$res = db_query($sql);
	if ($res) {
		$data = db_fetch_object($res);
		if ($data) {
			...
			if ($data->revisi=='1')	
				//1
				$tanggal = $data->dpatgl1;
			elseif ($data->revisi=='2')	
				//2
				$tanggal = $data->dpatgl2;
			elseif ($data->revisi=='3')	
				//3
				$tanggal = $data->dpatgl3;
			elseif ($data->revisi=='4')	
				//4
				$tanggal = $data->dpatgl4;
			else
				//5
				$tanggal = $data->dpatgl5;
		}
	}			
	return $tanggal;
}
